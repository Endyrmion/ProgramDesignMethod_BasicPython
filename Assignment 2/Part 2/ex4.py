# 4. The aspect ratio of an image describes the relationship between the width and height.
...
# Suppose you are writing a blog and you have an image that is m units wide and n units high but your blog only has space for an image that is z units wide (where z is less than m).
# Write a function called calc new height() that returns the height the image must be in order to preserve the aspect ratio (i.e., a height that will not distort the image).
...
def newHeightFunc():
    try:
        baseWidth = float(input("Width\n-> "))
        baseHeight = float(input("Height\n-> "))
        maxWidth = float(input("Updated Width\n-> "))
    except:
        print("Wrong Input ! ")
        exit()

    ratio = maxWidth / baseWidth
    newHeight = baseHeight * ratio
    print("The corresponding height is: %f" % (newHeight))
